export function atkinson(
  ...
  const factor = (options.factor || 1 / 8) * scale;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      adjustPixel(pixels, x + 1, y, quantError, factor, width, height);
      adjustPixel(pixels, x + 2, y, quantError, factor, width, height);
      adjustPixel(pixels, x - 1, y + 1, quantError, factor, width, height);
      adjustPixel(pixels, x, y + 1, quantError, factor, width, height);
      adjustPixel(pixels, x + 1, y + 1, quantError, factor, width, height);
      adjustPixel(pixels, x, y + 2, quantError, factor, width, height);
    }
  }
}
